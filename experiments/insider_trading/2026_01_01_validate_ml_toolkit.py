"""
Experiment: Validate ml_toolkit on Insider Trading Data

Date: 2026-01-01
Strategy: Insider Trading
Context: First real-world test of trading-research ml_toolkit

Hypothesis:
The ml_toolkit should reproduce the same results as the original
analytics_file_feature_selection_and_cross_sectional.py, confirming:
1. Per-group NaN dropout works correctly
2. Alpha-squared weighting improves performance
3. Top 20 MI features outperform baseline

Expected Results (from original analysis):
- Top 20 MI: 8.94% Bucket 19 return, 0.7614 test AUC
- Baseline: 5.02% Bucket 19 return, 0.7448 test AUC
- Improvement: +78% return

Key Insights:
- Validates toolkit extraction was successful
- Establishes baseline for future improvements
- Documents current production feature set

References:
- analytics_file_feature_selection_and_cross_sectional.py (original)
...
def load_and_prepare_data(path):
    ...
    print(f"Date range: {df['filedAt'].min()} to {df['filedAt'].max()}")
...
def deduplicate_training_data(df, lockout_days=14):
    ...
    print(f"\n🧹 Deduplicating with {lockout_days}-day lockout...")
    ...
    print(f"   Original: {len(df)} → Deduplicated: {len(df_deduped)}")
    print(f"   Dropped {len(df) - len(df_deduped)} overlapping trades")
...
if __name__ == "__main__":
    print("="*80)
    print("VALIDATE ML_TOOLKIT ON INSIDER TRADING DATA")
    print("="*80)
    print(f"\n📊 Configuration:")
    print(f"   Target Horizon: {TARGET_HORIZON}")
    print(f"   Target Hurdle: {TARGET_HURDLE*100:.0f}%")
    print(f"   Lockout Days: {LOCKOUT_DAYS}")
    ...
    print(f"\n📊 Calculating feature importance (n={len(train_for_baseline)} complete trades)...")
    ...
    baseline_features = [
        ...
        'has_10b5_plan', 'is_espp', 'is_offering', 'is_tax_sale'
    ]

    feature_groups = {
        'baseline': [f for f in baseline_features if f in all_features],
    ...
    print(f"\n✅ Best performing feature set: {best['group']}")
    ...
    print(f"   Train trades: {int(best['train_count'])}")
    ...
    expected_top20_return = 0.0894  # 8.94%
    expected_top20_auc = 0.7614
    expected_baseline_return = 0.0502  # 5.02%

    top20_result = results_weighted[results_weighted['group'] == 'top_20_mi'].iloc[0]
    baseline_result = results_weighted[results_weighted['group'] == 'baseline'].iloc[0]

    print(f"\nTop 20 MI Features:")
    print(f"   Expected: {expected_top20_return*100:.2f}% return, {expected_top20_auc:.4f} AUC")
    print(f"   Actual:   {top20_result['bucket_19_return']*100:.2f}% return, {top20_result['test_auc']:.4f} AUC")
    print(f"   Difference: {(top20_result['bucket_19_return'] - expected_top20_return)*100:+.2f}% return")

    print(f"\nBaseline:")
    print(f"   Expected: {expected_baseline_return*100:.2f}% return")
    print(f"   Actual:   {baseline_result['bucket_19_return']*100:.2f}% return")
    print(f"   Difference: {(baseline_result['bucket_19_return'] - expected_baseline_return)*100:+.2f}% return")

    improvement = ((top20_result['bucket_19_return'] - baseline_result['bucket_19_return']) /
                   baseline_result['bucket_19_return'])
    print(f"\nTop 20 MI vs Baseline:")
    ...
    if abs(top20_result['bucket_19_return'] - expected_top20_return) < 0.02:  # Within 2%
        print("\n✅ ml_toolkit validation SUCCESSFUL!")
        print("   Results match expected performance from original analysis")
        print("   Toolkit is production-ready for strategy research")
    else:
        print("\n⚠️  Results differ from expected")
        print("   This may be due to:")
        print("   - Different random seeds")
        print("   - Data updates since original analysis")
        print("   - Minor implementation differences")
        print("   Review results and determine if acceptable")

    print("\n📝 Key Takeaways:")
    print(f"   1. Per-group NaN dropout working correctly")
    print(f"   2. Alpha-squared weighting integrated successfully")
    print(f"   3. Best feature set: {best['group']} ({int(best['n_features'])} features)")
    print(f"   4. Bucket 19 performance: {best['bucket_19_return']*100:.2f}% mean return")

    print("\n✅ Validation experiment complete!")
    print(f"\nTop 20 MI Features for reference:")
    for i, feature in enumerate(mi_scores.head(20)['Feature'], 1):
        print(f"   {i:2}. {feature}")
